...
export function estimatedBrandShare(modalidade, brandId) {
  const active = activeBrandsFor(modalidade);
  const totalWeight = active.reduce((sum, b) => sum + (ESTIMATIVA_BANDEIRAS_BRASIL.pesos[b.id] ?? 0), 0);
  if (totalWeight <= 0 || !active.some((b) => b.id === brandId)) return 0;
  return ((ESTIMATIVA_BANDEIRAS_BRASIL.pesos[brandId] ?? 0) / totalWeight) * 100;
}
...
function brandAllocation(state, modalidade) {
  ...
  if (state.cards.origemMixBandeiras === 'estimativaBrasil') {
    return allocateCentsByShare(modalVol, active.map((b) => ({ id: b.id, share: estimatedBrandShare(modalidade, b.id) })));
  }
  if (state.cards.modoBandeiras === 'valor') return new Map(active.map((b) => [b.id, toCents(state.cards.modalities[modalidade]?.brands?.[b.id]?.valor) ?? 0]));
...
function brandDistributionValid(state, modalidade) {
  if (state.cards.detalharBandeiras === false || state.cards.origemMixBandeiras === 'estimativaBrasil') return true;
  const modalVol = modalityVolumeCents(state, modalidade); if (modalVol <= 0) return true; const active = activeBrandsFor(modalidade);
  if (state.cards.modoBandeiras === 'share') { const sum = active.reduce((s, b) => s + (parseDecimal(state.cards.modalities[modalidade]?.brands?.[b.id]?.share) ?? 0), 0); return Math.abs(sum - 100) < 0.01; }
  const sum = active.reduce((s, b) => s + brandVolumeCents(state, modalidade, b.id), 0); return Math.abs(sum - modalVol) <= 1;
}
...
export function calculateCards(state) {
  ...
  const mixEstimated = detailedBrands && state.cards.origemMixBandeiras === 'estimativaBrasil';
  ...
  if (!detailedBrands) {
    ...
    for (const modalidade of MODALIDADES) {
      ...
      for (const brand of activeBrandsFor(modalidade)) {
        ...
        detail.push({ modalidade, brandId: brand.id, bandeira: brand.nome, volumeCents, share: brandShare(state, modalidade, brand.id), taxaAtual: parseDecimal(cell.taxaAtual), taxaCielo: parseDecimal(cell.taxaCielo), currentMicro, proposedMicro, custoAtualCents: currentCents, custoCieloCents: proposedCents, impactoMensalCents: impactCents, impacto12Cents: impactCents === null ? null : impactCents * 12, complete: rowComplete, estimatedMix: mixEstimated });
      }
    }
  }
