...
export function fmtBRL(value) { const parsed = parseDecimal(value); return parsed === null ? '—' : nfBRL.format(parsed); }
...
export function fmtPct(value, decimals = 2) { const parsed = parseDecimal(value); if (parsed === null) return '—'; return `${parsed.toLocaleString('pt-BR', { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}%`; }
export function fmtPp(value, decimals = 2) { const parsed = parseDecimal(value); if (parsed === null) return '—'; return `${parsed.toLocaleString('pt-BR', { minimumFractionDigits: decimals, maximumFractionDigits: decimals })} p.p.`; }
export function fmtPoints(value) { const parsed = parseDecimal(value); if (parsed === null) return '—'; return `${nfInt.format(Math.round(parsed))} pts`; }
export function fmtDateBR(iso) { if (!iso) return '—'; const [y, m, d] = String(iso).slice(0, 10).split('-'); return y && m && d ? `${d}/${m}/${y}` : '—'; }
export function fmtNumber(value, decimals = 2) { const parsed = parseDecimal(value); if (parsed === null) return '—'; if (decimals === 0) return nfInt.format(Math.round(parsed)); return parsed.toLocaleString('pt-BR', { minimumFractionDigits: decimals, maximumFractionDigits: decimals }); }
